"""exp001: first live 5-agent run against OpenAI gpt-4.1-mini.

Objective: verify that the full OCT framework (environment + rules +
runner + 5 personas + dispatcher) actually produces sensible behavior
when driven by a real LLM, not just scripted fixtures.

Usage (from experiments/runtime/ with venv activated):
    # Put OPENAI_API_KEY=sk-... in experiments/runtime/.env
    python scripts/demo_five_agents.py

Outputs:
    - JSON trace saved to experiments/exp001_first_live_run/trace_seed42.json
    - Short human summary on stderr (steps, errors, snapshot, API calls)

Configuration is intentionally hard-coded here so the run is reproducible
by design: rng_seed=42, 15 days, temperature=0.8, 1 action/agent/day.
"""
...
# Experiment configuration (kept in-code for reproducibility)
...
# Output path: experiments/<exp_id>/ at the repo root (sibling of runtime/)
OUTPUT_DIR = (
    Path(__file__).resolve().parents[2] / EXPERIMENT_ID
)  # experiments/runtime/scripts/.. -> runtime/.. -> experiments/
