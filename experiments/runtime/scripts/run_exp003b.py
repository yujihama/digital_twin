"""exp003b: 承認フローベースライン確保（承認閾値50万円）.

exp003では承認閾値100万円のため、承認対象が1件（ノートPC 1,618,884円）のみだった。
介入実験（T-009: 閾値100万→500万）のベースラインとしては不十分。

exp003bでは承認閾値を50万円に引き下げ、複数件の承認/却下判断を発生させる。
これにより approver_c の承認行動パターンのベースラインを確立する。

exp003との差分:
  - EXPERIMENT_ID = "exp003b_approval_baseline"
  - approval_threshold = 500,000 (100万→50万に変更)
...
Outputs:
    - experiments/exp003b_approval_baseline/trace_seed42.json
    - stderr に実行サマリー
"""
...
# ---- Intervention: 承認閾値を50万円に引き下げ ----
APPROVAL_THRESHOLD = 500_000  # exp003は1,000,000 → 500,000に変更
...
def main() -> int:
    ...
    state.controls.approval_threshold = APPROVAL_THRESHOLD  # 50万円に変更
