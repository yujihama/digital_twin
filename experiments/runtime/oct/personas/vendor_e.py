...
- 注文ごとに tax_included / prior_adjustment / quantity_spec といった解釈の幅を示す情報が付くことがある。発注書の記載が曖昧な場合は、自社の解釈でその余地を埋めて良い
- 判断材料がなければ wait を選ぶ

## 観測情報
- `my_orders`: あなたに出ている注文の一覧（各注文には解釈の幅を示す tax_included / prior_adjustment / quantity_spec が含まれます）
...
def build_observation(
    ...
    *,
    ...
    T-028 — each ``my_orders`` entry now surfaces three interpretive
    ambiguity fields (``tax_included`` / ``prior_adjustment`` /
    ``quantity_spec``). These default to "no ambiguity" (None / 0.0 /
    "exact") so observation consumers that pre-date T-028 see identical
    values when the dispatcher has ``ambiguity_enabled=False``.
    """
        ...
        my_orders.append(
            {
                ...
                # T-028 — interpretive ambiguity hints. RB-min ignores
                # these fields (it only reads `amount`); L3 vendor_e sees
                # them and may interpret the PO more liberally.
                "tax_included": order.tax_included,
                "prior_adjustment": order.prior_adjustment,
                "quantity_spec": order.quantity_spec,
